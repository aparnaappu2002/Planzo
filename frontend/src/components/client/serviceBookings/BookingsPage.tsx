...
interface Booking {
  ...
  service: Service;
  vendor: Vendor;
  vendorApproval: string; // API returns string like "Pending" or "Approved"
                ...
                <tbody>
                  {data.Bookings.map((booking: Booking) => (
                    <tr key={booking._id} className="border-b border-yellow-200">
                      ...
                      <td className="px-4 py-2 text-gray-700">{booking.service.serviceTitle}</td>
                      <td className="px-4 py-2 text-gray-700">{booking.vendor.name}</td>
