...
export class JwtService implements IjwtInterface{
    ...
    generateResetToken(resetSecretKey: string, userId: string, email: string): string {
            return jwt.sign({userId, email, purpose: 'password_reset'}, resetSecretKey, {expiresIn: '15m'})
    
    }
    verifyPasswordResetToken(resetToken: string, resetSecretKey: string): { userId: string; email: string; purpose: string } | null {
        try{
            const decoded = jwt.verify(resetToken, resetSecretKey) as {userId: string, email: string, purpose: string}
